...
from myapp.ml.demand_predict import predict_demand
import time

def owner_predict_demand(request):
    ...
    if request.method == "POST":
        ...
        data = {
            ...
            "charging_power_kW": float(request.POST["charging_power_kW"]),
            "queue_length": int(request.POST["queue_length"]),
            "station_load": float(request.POST["station_load"]),
            "electricity_price": float(request.POST["electricity_price"]),
            "renewable_energy_ratio": float(request.POST["renewable_energy_ratio"]),
            "traffic_density": request.POST["traffic_density"],
        ...
        if prediction < 20:
            demand_level = "LOW"
            recommendation = "Current infrastructure is sufficient."

        elif prediction < 35:
            demand_level = "MEDIUM"
            recommendation = "Monitor station usage."

        else:
            demand_level = "HIGH"
            recommendation = "Increase active chargers and power allocation."

    return render(
        request,
        "EV_Owner/predict_demand.html",
        {
